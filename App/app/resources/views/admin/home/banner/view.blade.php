		...
		<div class="row">
			...
			<div class="col-sm-6">
                <div class="bookmark pull-right">
                    <a href="{{url('/')}}/admin/home/banner/upload" class="btn btn sm btn-outline-dark" id="btnUploadBanner">Upload</a>
                </div>
			</div>
		</div>
	</div>
					...
					@for($i=0;$i<count($bannerImages);$i++)
						<figure class="col-xl-4 col-sm-6" data-tran-no="{{$bannerImages[$i]->TranNo}}" >
							<a href="{{url('/')}}/{{$bannerImages[$i]->BannerImage}}" data-lightbox="banner-images">
								<img src="{{url('/')}}/{{$bannerImages[$i]->BannerImage}}"  alt="Image {{$i}}">
							</a>
...
<script>
	$(document).ready(function(){
		...
		$(document).on('click','.btnEdit',function(){
			window.location.replace("{{url('/')}}/admin/home/banner/edit/"+ $(this).attr('data-id'));
		});
		$(document).on('click','.btnDelete',function(e){
            ...
            swal({
                ...
                $.ajax({
                    ...
                    url: "{{url('/')}}/admin/home/banner/delete/"+ id,

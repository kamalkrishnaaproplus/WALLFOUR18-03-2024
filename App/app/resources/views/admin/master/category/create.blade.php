				...
				<div class="col-sm-12">
					<div class="card">
						...
						<div class="card-body">
                            <div class="row  d-flex justify-content-center">
                                <div class="col-sm-7">
                                    <input type="file" id="txtCategoryImage" class="dropify" data-max-file-size="{{$Settings['upload-limit']}}" data-default-file="<?php if($isEdit==true){if($EditData[0]->CImage !=""){ echo url('/')."/".$EditData[0]->CImage;}}?>"  data-allowed-file-extensions="jpeg jpg png gif" />
                            ...
                            <div class="row mt-20">
                                <div class="col-md-12">
                                    <div class="form-group">
                                        <label for="txtCategoryName">Category Name <span class="required">*</span></label>
                                        <input type="text" id="txtCategoryName" class="form-control" placeholder="Category Name" value="<?php if($isEdit==true){ echo $EditData[0]->CName;} ?>">
                        ...
                        <div class="card-footer">
                            <div class="row">
                                <div class="col-sm-12 text-right">
                                    @if($crud['view']==1)
                                        <a href="{{ url('/') }}/admin/master/category" class="btn btn-sm btn-outline-dark mr-10">Cancel</a>
                                    @endif
...
<script>
    $(document).ready(function(){
        const formValidation=()=>{
            ...
            let status=true;
            if(CName==""){
                $('#txtCategoryName-err').html('Category Name is required');status=false;
            }else if(CName.length<2){
                $('#txtCategoryName-err').html('The Category Name is must be greater than 2 characters.');status=false;
            }else if(CName.length>100){
                $('#txtCategoryName-err').html('The Category Name is not greater than 100 characters.');status=false;
            }
            return status;
        }
		const getData=()=>{
        ...
        $(document).on('click','#btnSave',function(){
            ...
            if(status){
                ...
                function(){
                    swal.close();
                    btnLoading($('#nextBtn'));
                    @if($isEdit) let posturl="{{url('/')}}/admin/master/category/edit/{{$CID}}"; @else let posturl="{{url('/')}}/admin/master/category/create"; @endif
                    $.ajax({
                        type:"post",
                        url:posturl,
                        ...
                        success:function(response){
                            ...
                            if(response.status==true){
                                swal({
                                    ...
                                    @if($isEdit==true)
                                        window.location.replace("{{url('/')}}/admin/master/category/");
                                    @else
                                ...
                                if(response['errors']!=undefined){
                                    ...
                                    $.each( response['errors'], function( KeyName, KeyValue ) {
                                        var key=KeyName;
                                        if(key=="CategoryName"){$('#txtCategoryName-err').html(KeyValue);}
                                        if(key=="CategoryImage"){$('#txtCategoryImage-err').html(KeyValue);}
                                    });
                                }
                            }
                        }

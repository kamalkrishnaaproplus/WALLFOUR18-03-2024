	...
	<div id="service-category" class="container">
		<div class="row">
			<div id="content" class="col">
				...
				<div id="service-list" class="service-grid row row-cols-2 row-cols-sm-2 row-cols-md-3 row-cols-lg-3 row-cols-xl-4">
...
<script>
    $(document).ready(function(){
        let TotalServices=parseInt("{{$ServicesCount}}");
        let currentPage=1;
        let limitPerPage=8;
		var totalPages = Math.ceil(TotalServices / limitPerPage);
		var paginationSize = 7;
        const showPageInfo=()=>{
			let from =(currentPage-1)*limitPerPage;
			let to =from+limitPerPage;
			to=TotalServices<to?TotalServices:to;
			let text="Showing "+(from+1)+" to "+to+" of "+TotalServices+" ("+totalPages+" Pages)";

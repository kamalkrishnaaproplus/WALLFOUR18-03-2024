...
class clientController extends Controller
{
    ...
    {
        $sql = "SELECT P.ServiceID, P.ServiceName, P.Slug, P.ServiceImage, P.CID, C.CName, P.Price, P.UOM as UID, U.UCode, U.UName, P.Decimals, P.TaxID, P.TaxType, P.HSNSAC, P.ShortDescription, P.Description, P.ActiveStatus FROM tbl_services as P LEFT JOIN tbl_category as C ON C.CID=P.CID LEFT JOIN tbl_uom as U ON U.UID=P.UOM LEFT JOIN tbl_tax as T ON T.TaxID=P.TaxID Where P.DFlag=0 ";
        if (is_array($data)) {
...
public function Save(Request $req)
{
    if ($this->general->isCrudAllow($this->CRUD, "add") == true) {
        ...
        if ($status == true) {
            ...
            return array('status' => true, 'message' => "Service Create Successfully");
        } else {
            if ($ServiceImage != "") {
        ...
        if ($this->general->isCrudAllow($this->CRUD, "edit") == true) {
            ...
            try {
                ...
                if (array_key_exists("coverImg", $img)) {
                    if (file_exists($img['coverImg']['uploadPath'])) {
                        $ServiceImage = $this->addWaterMark->addWaterMark($img['coverImg']['uploadPath'], $dir, $img['coverImg']['fileName'], true);
                        $data['ServiceImage'] = $dir . $ServiceImage; // Concatenate directory with file name
                        unlink($img['coverImg']['uploadPath']);
                        $t = DB::table('tbl_services')->where('ServiceID', $ServiceID)->get();
                        if (count($t) > 0) {
                            $CServiceImage = $t[0]->ServiceImage;
                        }
                    }
                }
            ...
            if ($status == true) {
                DB::commit();
                if ($CServiceImage != "") {
                    if (file_exists($CServiceImage)) {
                        unlink($CServiceImage);
                    }
                }
                $NewData = $this->getServices(array("ServiceID" => $ServiceID));

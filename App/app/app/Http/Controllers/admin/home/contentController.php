...
class contentController extends Controller{
	...
	public function Update(Request $req,$TranNo){
		if($this->general->isCrudAllow($this->CRUD,"edit")==true){
			...
			$rules=array(
				'SectionName' =>['required','min:3','max:100',new ValidUnique(array("TABLE"=>"tbl_home_contents","WHERE"=>" SectionName='".$req->SectionName."' and TranNo<>'".$TranNo."'"),"This Section Name is already exists.")],
			);
			$message=array();
			...
			try{
				...
				$status=DB::Table('tbl_home_contents')->where('TranNo',$TranNo)->update($data);
				if($status){
					$this->updateOrderBy($req->Position,"Begining",true);
				}
			}catch(Exception $e) {
				$status=false;
			}
			if($status==true){
				DB::commit();
				$NewData=DB::Table('tbl_home_contents')->where('TranNo',$TranNo)->get();
				$logData=array("Description"=>"Content updated ","ModuleName"=>$this->ActiveMenuName,"Action"=>cruds::UPDATE,"ReferID"=>$TranNo,"OldData"=>$OldData,"NewData"=>$NewData,"UserID"=>$this->UserID,"IP"=>$req->ip());
				$this->logs->Store($logData);
